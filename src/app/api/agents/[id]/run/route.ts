...
export async function POST(
  ...
  try {
    ...
    let taskResult: { taskId: string; taskTitle: string; output: string } | null = null;
    if (agent.tasks.length > 0) {
      ...
      // Also save as an artifact for the Dev Server if output looks like code
      const codeIndicators = ['<html', '<!doctype', '<div', '<body', 'function ', 'const ', 'import ', 'export ', 'class ', '{\n', 'def ', 'return ', '.css', '{\n '];
      const looksLikeCode = codeIndicators.some(ind => taskOutput.toLowerCase().includes(ind.toLowerCase()));
      if (looksLikeCode || topTask.tags?.includes('code') || topTask.tags?.includes('html') || topTask.tags?.includes('frontend')) {
        let artifactType = 'code';
        if (taskOutput.toLowerCase().includes('<!doctype') || taskOutput.toLowerCase().includes('<html')) artifactType = 'html';
        else if (taskOutput.toLowerCase().includes('import react') || taskOutput.includes('jsx')) artifactType = 'react';
        else if (taskOutput.trim().startsWith('{') || taskOutput.trim().startsWith('[')) artifactType = 'json';
        else if (taskOutput.includes('{') && taskOutput.includes(':') && !taskOutput.includes('<')) artifactType = 'css';

        await db.artifact.create({
          data: {
            title: `Agent Output: ${topTask.title}`,
            description: `Auto-generated by ${agent.name} from task execution`,
            type: artifactType,
            content: taskOutput,
            status: 'ready',
            agentId: agent.id,
            taskId: topTask.id,
            tags: 'auto-generated,agent-output',
          },
        });
      }
